...
{     
    ...
    public class Scene
    {
        ...
        private void Add(CharacterDef character, CharacterLocation location)
        {
            ...
            if (CharactersPresent.Values.Select(x => x.Def).Contains(character)) 
                throw new Exception($"The scene {Name} already contains the character {character.Name}!");
            if (CharactersPresent.ContainsKey(location)) 
                throw new Exception($"The scene {Name} already contains a character at {location}!");
            CharactersPresent[location] = character.InstantiateCharacterAt(location).GetComponent<Character>();
        }
        /// <summary>
